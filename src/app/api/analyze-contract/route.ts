...
import {
  getDefaultMockResult,
  ReceptionAnalysisResult,
} from "../../../data/mockAnalysisResults";
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      console.log(
        `Attempt ${
          attempt + 1
        }/${maxRetries} - Making request to reCeption API...`
      );
      ...
      console.log(
        `Attempt ${attempt + 1} - Response status:`,
        response.status,
        response.statusText
      );
...
export async function POST() {
  try {
      ...
      return NextResponse.json(
        {
          ...
          userMessage:
            "Contract analysis service is not properly configured. Please contact support.",
        },
        { status: 500 }
      );
    }
    ...
    console.log("reCeption API response:", response);
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500) {
        userMessage =
          "Contract analysis service is temporarily unavailable. Please try again later.";
      } else if (response.status === 401 || response.status === 403) {
        userMessage =
          "Contract analysis service authentication failed. Please contact support.";
      }
    ...
    if (isConnectionError) {
      console.log(
        "API connection failed, falling back to default analysis data..."
      );
